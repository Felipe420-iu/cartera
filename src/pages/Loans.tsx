...
const Loans: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (storedLoans) {
      try {
          ...
          console.error('No se pudieron leer los prestamos', err)
      }
    }
  }, [])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!clientId || !amount || !installmentAmount || !installments || !startDate) {
      setError('Completa todos los campos del prestamo')
      return
    }
  ...
  const getLoanMetrics = (loan: LocalLoan) => {
    ...
    const pending = total - paid
        ...
        const status = hasLate ? 'EN MORA' : 'AL DIA'
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button className="btn-primary" onClick={() => setShowForm(!showForm)}>
              {showForm ? 'Cerrar' : 'Nuevo Prestamo'}
        </button>
      </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Recargo por mora (opcional y manual)</h3>
              <p className="text-sm text-gray-600">Solo se puede aplicar si la cuota lleva mas de 3 dias vencida.</p>
          </div>
      ...
      {showForm && (
        <div className="card">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Datos del prestamo</h3>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 flex gap-3">
              <button type="submit" className="btn-primary">Crear Prestamo</button>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900">Prestamos registrados</h3>
                ...
                {loans.map(loan => {
                  ...
                  return (
                    <tr key={loan.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-900">
                        {loan.frequency === 'daily' ? 'Diario' : loan.frequency === 'weekly' ? 'Semanal' : 'Mensual'}
                      </td>
      ...
      {loans.length > 0 && (
        <div className="card">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Calendario de pagos generado</h3>
                        ...
                        {loan.schedule.map(inst => {
                          ...
                          return (
                            <tr key={inst.installmentNumber}>
                              ...
                              <td className="px-3 py-2 text-gray-900">
                                {renderStatusChip(inst)}
                                {status === 'LATE' && (
                                  <span className="ml-2 text-[11px] text-red-700">({overdueDays} dias)</span>
                                )}
                              </td>
