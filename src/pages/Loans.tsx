...
const Loans: React.FC = () => {
  ...
  const buildSchedule = (start: string, installments: number, freq: Frequency, installmentAmount: number) => {
    const schedule: LocalInstallment[] = []
    let cursor = new Date(start)
  ...
  const applyRecargo = (loanId: number, installmentNumber: number, recargo: number) => {
    setLoans(prev => prev.map(loan => {
      if (loan.id !== loanId) return loan
      const schedule = loan.schedule.map(inst => {
        if (inst.installmentNumber !== installmentNumber) return inst
        ...
        return {
          ...inst,
          amount: inst.amount + recargo,
          recargoApplied: true,
          recargoAmount: recargo
        }
      })
      return { ...loan, schedule }
    }))
  ...
  const getLoanMetrics = (loan: LocalLoan) => {
    const recargos = loan.schedule.reduce((sum, inst) => sum + (inst.recargoAmount || 0), 0)
    const paid = loan.schedule.filter(inst => inst.status === 'PAID').reduce((sum, inst) => sum + inst.amount, 0)
    const total = loan.totalToPay + recargos
    const pending = total - paid
    const hasLate = loan.schedule.some(inst => deriveStatus(inst) === 'LATE')
    const status = hasLate ? 'EN MORA' : 'AL DÍA'
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Préstamos</h1>
          <p className="text-gray-600 text-sm">Modelo simple: prestas un monto y te devuelven un monto fijo mayor.</p>
        </div>
        <button className="btn-primary" onClick={() => setShowForm(!showForm)}>
          {showForm ? 'Cerrar' : 'Nuevo Préstamo'}
        </button>
                            ...
                            <tr key={inst.installmentNumber}>
                              ...
                              <td className="px-3 py-2 text-gray-900 space-x-2">
                                ...
                                {canApplyRecargo && (
                                  <button
                                    className="text-red-600 hover:text-red-800 text-xs"
                                    onClick={() => applyRecargo(loan.id, inst.installmentNumber, recargoFijo)}
                                  >
                                    Aplicar recargo
                                  </button>
                                )}
